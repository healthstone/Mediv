    ...
    bool OnGossipHello(Player *player) override {
        ...
        if (CanBuild()) {
            if (player->HasAura(SPELL_CORPORAL))
                AddGossipItemFor(player, GOSSIP_ICON_CHAT, GOSSIP_HELLO_DEMO1, GOSSIP_SENDER_MAIN,
                                 GOSSIP_ACTION_INFO_DEF);
            else if (player->HasAura(SPELL_LIEUTENANT)) {
                AddGossipItemFor(player, GOSSIP_ICON_CHAT, GOSSIP_HELLO_DEMO1, GOSSIP_SENDER_MAIN,
                                 GOSSIP_ACTION_INFO_DEF);
                AddGossipItemFor(player, GOSSIP_ICON_CHAT, GOSSIP_HELLO_DEMO2, GOSSIP_SENDER_MAIN,
                                 GOSSIP_ACTION_INFO_DEF + 1);
                AddGossipItemFor(player, GOSSIP_ICON_CHAT, GOSSIP_HELLO_DEMO3, GOSSIP_SENDER_MAIN,
                                 GOSSIP_ACTION_INFO_DEF + 2);
            }
        } else
            AddGossipItemFor(player, GOSSIP_ICON_CHAT, GOSSIP_HELLO_DEMO4, GOSSIP_SENDER_MAIN,
        ...
        if (CanBuild()) {
            switch (action - GOSSIP_ACTION_INFO_DEF) {
                ...
                case 1:
                    ...
                    break;
                case 2:
                    DoCast(player, player->GetTeamId() == TEAM_ALLIANCE ? SPELL_BUILD_SIEGE_VEHICLE_FORCE_ALLIANCE
                                                                        : SPELL_BUILD_SIEGE_VEHICLE_FORCE_HORDE, true);
                    break;
            }
    ...
    bool OnGossipHello(Player *player) override {
        ...
        for (uint8 i = 0; i < graveyard.size(); i++)
            if (graveyard[i]->GetControlTeamId() == player->GetTeamId())
                AddGossipItemFor(player, GOSSIP_ICON_CHAT, player->GetSession()->GetTrinityString(
                                         ((WintergraspGraveyard *) graveyard[i])->GetTextId()), GOSSIP_SENDER_MAIN,
    ...
    bool OnGossipHello(Player *player) override {
        ...
        if (wintergrasp->IsWarTime()) {
            AddGossipItemFor(player, GOSSIP_ICON_CHAT,
                             player->GetSession()->GetTrinityString(WG_NPCQUEUE_TEXTOPTION_JOIN), GOSSIP_SENDER_MAIN,
                             GOSSIP_ACTION_INFO_DEF);
            SendGossipMenuFor(player, wintergrasp->GetDefenderTeam() ? WG_NPCQUEUE_TEXT_H_WAR : WG_NPCQUEUE_TEXT_A_WAR,
                              me->GetGUID());
        } else {
            ...
            if (timer < 15 * MINUTE) {
                AddGossipItemFor(player, GOSSIP_ICON_CHAT,
                                 player->GetSession()->GetTrinityString(WG_NPCQUEUE_TEXTOPTION_JOIN),
...
class spell_wintergrasp_force_building : public SpellScript {
    ...
    void Register() override {
        OnEffectHitTarget += SpellEffectFn(spell_wintergrasp_force_building::HandleScript, EFFECT_0,
...
class spell_wintergrasp_grab_passenger : public SpellScript {
    ...
    void Register() override {
        OnEffectHitTarget += SpellEffectFn(spell_wintergrasp_grab_passenger::HandleScript, EFFECT_0,
                                           SPELL_EFFECT_SCRIPT_EFFECT);
    }
};
    ...
    SpellCastResult CheckCast() {
        if (Battlefield *wg = sBattlefieldMgr->GetBattlefieldByBattleId(BATTLEFIELD_BATTLEID_WG))
            if (Player *target = GetExplTargetUnit()->ToPlayer())
                ...
                if ((target->GetZoneId() == AREA_WINTERGRASP && target->GetTeamId() != wg->GetDefenderTeam()) ||
                    target->HasAura(SPELL_WINTERGRASP_TELEPORT_TRIGGER))
...
class spell_wintergrasp_defender_teleport_trigger : public SpellScript {
    ...
    void Register() override {
        OnEffectHitTarget += SpellEffectFn(spell_wintergrasp_defender_teleport_trigger::HandleDummy, EFFECT_0,
                                           SPELL_EFFECT_DUMMY);
    }
};
...
class spell_wintergrasp_tenacity_refresh : public AuraScript {
    ...
    void Register() override {
        OnEffectPeriodic += AuraEffectPeriodicFn(spell_wintergrasp_tenacity_refresh::Refresh, EFFECT_2,
                                                 SPELL_AURA_PERIODIC_DUMMY);
        AfterEffectRemove += AuraEffectRemoveFn(spell_wintergrasp_tenacity_refresh::OnRemove, EFFECT_2,
