...
constexpr uint32 MAX_NETCLIENT_PACKET_SIZE =
        32767 - 1; // Client hardcap: int16 with trailing zero space otherwise crash on memory free
...
void MailMgr::Update(uint32 diff) {
    ...
    {
        _ExpTimer.Update(int32(m_updateTimer / 1000));
        if (_ExpTimer.Passed())
            _DeleteExpiryMails();
...
void
MailMgr::SendMailBy(Object *sender, ObjectGuid::LowType receiver, std::string const &subject, std::string const &body,
                    uint32 money, MailCheckMask mask, uint32 deliver_delay, uint32 COD) {
    if (!sender || !receiver)
        return;
    ...
    if (!PrepareMessageAttributeBy(sender, receiver, m_messageType, m_stationery, m_senderId, deliver_delay, COD,
                                   expire_time, deliver_time))
        return;

    uint32 mail_id = AddNewMail(m_messageType, m_stationery, 0, m_senderId, receiver, subject, body, false, money,
                                expire_time, deliver_time, COD, mask);
...
void MailMgr::SendMailByGUID(ObjectGuid::LowType sender, ObjectGuid::LowType receiver, uint8 m_messageType,
                             std::string const &subject, std::string const &body, uint32 money, MailCheckMask mask,
    ...
    PrepareMessageAttributeByGUID(sender, m_messageType, m_stationery, deliver_delay, COD, expire_time, deliver_time);

    uint32 mail_id = AddNewMail(m_messageType, m_stationery, 0, sender, receiver, subject, body, false, money,
                                expire_time, deliver_time, COD, mask);
...
void MailMgr::SendMailWithItemsBy(Object *sender, ObjectGuid::LowType receiver, std::string const &subject,
                                  std::string const &body, uint32 money, std::vector<Item *> const &itemlist,
                                  MailCheckMask mask, uint32 deliver_delay, uint32 COD) {
    if (!sender || !receiver)
        return;
    ...
    uint32 mail_id = AddNewMail(m_messageType, m_stationery, 0, m_senderId, receiver, subject, body, true, money,
                                expire_time, deliver_time, COD, mask);
...
void MailMgr::SendMailWithItemsByGUID(ObjectGuid::LowType sender, ObjectGuid::LowType receiver, uint8 m_messageType,
                                      std::string const &subject, std::string const &body, uint32 money,
                                      std::vector<Item *> const &itemlist, MailCheckMask mask, uint32 deliver_delay,
                                      uint32 COD) {
    if (!receiver)
        return;
    ...
    PrepareMessageAttributeByGUID(sender, m_messageType, m_stationery, deliver_delay, 0, expire_time,
                                  deliver_time);

    uint32 mail_id = AddNewMail(m_messageType, m_stationery, 0, sender, receiver, subject, body, true, money,
                                expire_time, deliver_time, COD, mask);
...
void MailMgr::SendMailByAuctionHouse(AuctionEntry *sender, ObjectGuid::LowType receiver, std::string const &subject,
                                     std::string const &body, uint32 money, MailCheckMask mask, uint32 deliver_delay,
                                     uint32 COD) {
    if (!sender || !receiver)
        return;
    ...
    PrepareMessageAttributeForAuctionAndCalendar(deliver_delay, COD, expire_time, deliver_time);
...
void
MailMgr::SendMailByAuctionHouseWithItems(AuctionEntry *sender, ObjectGuid::LowType receiver, std::string const &subject,
                                         std::string const &body, uint32 money, std::vector<Item *> const &itemlist,
                                         MailCheckMask mask, uint32 deliver_delay, uint32 COD) {
    if (!sender || !receiver)
        return;
...
void MailMgr::SendMailByCalendarEvent(CalendarEvent *sender, ObjectGuid::LowType receiver, std::string const &subject,
                                      std::string const &body, uint32 money, MailCheckMask mask, uint32 deliver_delay,
                                      uint32 COD) {
    if (!sender || !receiver)
        return;
...
void MailMgr::SendMailByCalendarEventWithItems(CalendarEvent *sender, ObjectGuid::LowType receiver,
                                               std::string const &subject, std::string const &body, uint32 money,
                                               std::vector<Item *> const &itemlist, MailCheckMask mask,
                                               uint32 deliver_delay, uint32 COD) {
    if (!sender || !receiver)
        return;
...
void
MailMgr::SendMailWithTemplateBy(Object *sender, ObjectGuid::LowType receiver, uint16 mailTemplateId, MailCheckMask mask,
    ...
    if (!PrepareMessageAttributeBy(sender, receiver, m_messageType, m_stationery, m_senderId, deliver_delay, 0,
                                   expire_time, deliver_time))
        return;
    ...
    uint32 mail_id = AddNewMail(m_messageType, m_stationery, mailTemplateId, m_senderId, receiver, "", "", false,
                                m_money, expire_time, deliver_time, 0, mask);
...
void MailMgr::SendMailWithTemplateByGUID(ObjectGuid::LowType sender, ObjectGuid::LowType receiver, uint8 messageType,
                                         uint16 mailTemplateId, MailCheckMask mask, uint32 deliver_delay) {
    if (!sender || !receiver)
        return;
    ...
    PrepareMessageAttributeByGUID(sender, messageType, m_stationery, deliver_delay, 0, expire_time, deliver_time);

    // The mail sent after turning in the quest The Good News and The Bad News contains 100g
    if (mailTemplateId == 123)
        m_money = 1000000;

    uint32 mail_id = AddNewMail(messageType, m_stationery, mailTemplateId, sender, receiver, "", "", false, m_money,
                                expire_time, deliver_time, 0, mask);
    ...
    for (uint32 i = 0; /*m_items.size() < MAX_MAIL_ITEMS &&*/ i < max_slot; ++i) {
        if (LootItem *lootitem = mailLoot.LootItemInSlot(i, receiver)) {
            if (Item *item = Item::CreateItem(lootitem->itemid, lootitem->count, receiver)) {
                item->SaveToDB(
                        trans);                           // save for prevent lost at next mail load, if send fail then item will deleted
...
uint32 MailMgr::SendReturnMailByGUID(uint32 old_mailID, ObjectGuid::LowType sender, ObjectGuid::LowType receiver,
                                     std::string const &subject, std::string const &body, uint32 money, bool itemexist,
                                     uint32 deliver_delay) {
    ...
    uint32 mail_id = AddNewMail(m_messageType, m_stationery, 0, sender, receiver, subject, body, itemexist, money,
                                expire_time, deliver_time, 0, mask);
...
void
MailMgr::clearDependInstanceItem(ObjectGuid::LowType playerId, uint32 mailID, CharacterDatabaseTransaction &trans) {
...
bool MailMgr::PrepareMessageAttributeBy(Object *sender, ObjectGuid::LowType receiver, uint8 &m_messageType,
                                        uint8 &m_stationery, ObjectGuid::LowType &m_senderId, uint32 deliver_delay,
                                        uint32 COD, time_t &expire_time, time_t &deliver_time) {
    ...
    switch (sender->GetTypeId()) {
        ...
        default:
            ...
            TC_LOG_ERROR("misc", "MailMgr::SendMailTo - Mail message contains unexpected sender typeid ({}).",
                         sender->GetTypeId());
            typeIsOk = false;
            break;
    }
...
void MailMgr::PrepareMessageAttributeByGUID(ObjectGuid::LowType senderEntry, uint8 m_messageType, uint8 &m_stationery,
                                            uint32 deliver_delay, uint32 COD, time_t &expire_time,
                                            time_t &deliver_time) {
    m_stationery = MAIL_STATIONERY_DEFAULT;
    if (!senderEntry)
        m_stationery = MAIL_STATIONERY_GM;
...
void MailMgr::PrepareMessageAttributeForAuctionAndCalendar(uint32 deliver_delay, uint32 COD,
                                                           time_t &expire_time,
                                                           time_t &deliver_time) {
    deliver_time = GameTime::GetGameTime() + deliver_delay;
    //expire time if COD 3 days, if no COD 30 days, if auction sale pending 1 hour
...
uint32 MailMgr::AddNewMail(uint8 messageType, uint8 stationery, uint16 mailTemplateId, ObjectGuid::LowType sender,
                           ObjectGuid::LowType receiver,
                           std::string const &subject, std::string const &body, bool item_exists, uint32 money,
                           time_t expire_time, time_t deliver_time,
    ...
    stmt->setInt8(++index, int8(stationery));
    ...
    if (expire_time - now < currentExpTime)
        _ExpTimer.Reset(int32(expire_time - now));
...
void MailMgr::RemoveMail(uint32 mailID, CharacterDatabaseTransaction &trans) {
    ...
    auto mailItr = m_mails.find(mailID);
    if (mailItr != m_mails.end()) {
        delete mailItr->second;
    }
...
MailMgr::AddNewMailItem(uint32 mailID, Item *itemPointer, ObjectGuid::LowType itemGuidLow, ObjectGuid::LowType receiver,
    ...
    if (itemexist) {
        TC_LOG_ERROR("mailMgr",
                     "New Mail ({}) :: AddNewMailItem (GUID: {}), but item with this item_guid already exist, skip",
                     mailID, itemGuidLow);
        return;
    }
...
void MailMgr::RemoveMailItemsByMailId(uint32 mailID, CharacterDatabaseTransaction &trans) {
    ...
    for (auto itr: miID) {
        if (itr) {
            auto itemItr = m_mailitems.find(itr);
            if (itemItr != m_mailitems.end()) {
...
bool MailMgr::HandleMailDelete(uint32 mailID) {
    ...
    if (itr != m_mails.end()) {
        ...
        if (itr->second->COD && itr->second->sender && itr->second->messageType == MAIL_NORMAL)
            SendMailByGUID(0, itr->second->sender, MAIL_NORMAL, "Money received", "From old mail with COD",
                           itr->second->COD, MAIL_CHECK_MASK_COD_PAYMENT);
    }
...
MailResponseResult MailMgr::HandleMailReturnToSender(uint32 mailID) {
    ...
    auto const itr = m_mails.find(mailID);
    if (itr != m_mails.end()) {
        ...
        if (itr->second->sender) // new receiver
        {
            ...
            if (Player *receiver = ObjectAccessor::FindPlayerByLowGUID(itr->second->sender)) {
                if (GetMailBoxSize(itr->second->sender) + receiver->GetAditionalData()->getAuctionLotsCount() >
                    sWorld->customGetIntConfig(CONFIG_ANTISPAM_MAIL_COUNT_CONTROLLER))
    ...
    uint32 new_mailID = SendReturnMailByGUID(mailID, new_sender, new_receiver, subject, body, money, itemsExist,
                                             deliver_delay);
...
MailResponseResult
MailMgr::HandleMailTakeItem(Player *player, uint32 mailID, ObjectGuid::LowType item_guid, uint32 &count,
                            uint32 &msg_result) {
    ...
    auto mailItr = m_mails.find(mailID);
    if (mailItr != m_mails.end()) {
        if (mailItr->second->deliver_time > GameTime::GetGameTime())
            return MAIL_ERR_INTERNAL_ERROR;

        // check on item id
        bool itemexist = false;
        for (auto itemItr: m_mailitems) {
            if (itemItr.second->messageID == mailID && itemItr.second->item_guid == item_guid)
                itemexist = true;
        }
        ...
        if (!it || !itemexist) {
            if (!it && itemexist)
                TC_LOG_ERROR("mailMgr", "Mail ({}) has item (GUID: {}), but doesn not exist in ItemMap of pointers",
                             mailID, item_guid);
            if (it && !itemexist)
                TC_LOG_ERROR("mailMgr", "Mail ({}) has item (GUID: {}), but doesn not exist in MailItemMap", mailID,
                             item_guid);
            return MAIL_ERR_MAIL_ATTACHMENT_INVALID;
        }

        if (!player->HasEnoughMoney(mailItr->second->COD))
        ...
        if (msg_result == EQUIP_ERR_OK) {
            ...
            if (mailItr->second->COD > 0) {
                ObjectGuid sender_guid(HighGuid::Player, mailItr->second->sender);
                ...
                if (player->GetSession()->HasPermission(rbac::RBAC_PERM_LOG_GM_TRADE)) {
                    ...
                    sLog->OutCommand(player->GetSession()->GetAccountId(),
                                     ...
                                     player->GetName(), player->GetSession()->GetAccountId(), it->GetTemplate()->Name1,
                                     it->GetEntry(), it->GetCount(), mailItr->second->COD, sender_name, sender_accId);
                } else if (!receiver)
                    sender_accId = sCharacterCache->GetCharacterAccountIdByGuid(sender_guid);

                // check player existence
                if (receiver || sender_accId)
                    SendMailBy(player, mailItr->second->sender, mailItr->second->subject, "", mailItr->second->COD,
                               MAIL_CHECK_MASK_COD_PAYMENT);

                player->ModifyMoney(-int32(mailItr->second->COD));

                mailItr->second->COD = 0;
            }

            //upd state
            mailItr->second->state = MAIL_STATE_CHANGED;
            ...
            it->SetState(
                    ITEM_UNCHANGED);                       // need to set this state, otherwise item cannot be removed later, if neccessary
            ...
            CharacterDatabasePreparedStatement *stmt3 = CharacterDatabase.GetPreparedStatement(
...
MailResponseResult MailMgr::HandleMailTakeMoney(Player *player, uint32 mailID, uint32 &msg_result) {
    ...
    if (itr != m_mails.end()) {
        ...
        if (!player->ModifyMoney(int32(itr->second->money), false)) {
...
void MailMgr::HandleGetMailList(Player *player, WorldPacket &data) {
    ...
    for (auto mailItr: m_mails) {
        if (mailItr.second->receiver == player->GetGUID().GetCounter()) {
            ...
            if (cur_time < mailItr.second->deliver_time)
                continue;
            ...
            size_t next_mail_size = 2 + 4 + 1 + (mailItr.second->messageType == MAIL_NORMAL ? 8 : 4) + 4 * 8 +
                                    (mailItr.second->subject.size() + 1) + (mailItr.second->body.size() + 1) + 1 +
                                    item_count *
                                    (1 + 4 + 4 +
            ...
            data << uint32(mailItr.first);                        // Message ID
            data << uint8(mailItr.second->messageType);            // Message Type

            switch (mailItr.second->messageType) {
                case MAIL_NORMAL:                              // sender guid
                    data << ObjectGuid(HighGuid::Player, mailItr.second->sender);
                    break;
                case MAIL_CREATURE:
                ...
                case MAIL_CALENDAR:
                    data << uint32(
                            mailItr.second->sender);              // creature/gameobject entry, auction id, calendar event id?
                    break;
            }

            data << uint32(mailItr.second->COD);                         // COD
            data << uint32(0);                                       // package (Package.dbc)
            data << uint32(mailItr.second->stationery);                  // stationery (Stationery.dbc)
            data << uint32(mailItr.second->money);                       // Gold
            data << uint32(mailItr.second->checked);                     // flags
            data << float(mailItr.second->expire_time - GameTime::GetGameTime()) / float(DAY); // Time
            data << uint32(mailItr.second->mailTemplateId);              // mail template (MailTemplate.dbc)
            data
                    << mailItr.second->subject;                             // Subject string - once 00, when mail type = 3, max 256
            data << mailItr.second->body;                                // message? max 8000
    ...
    data.put<uint32>(0,
                     realCount); // this will display warning about undelivered mail to player if realCount > mailsCount
...
MailResponseResult MailMgr::HandleMailCreateTextItem(Player *player, uint32 mailID, uint32 &msg_result) {
    ...
    auto itr = m_mails.find(mailID);
    if (itr != m_mails.end()) {
        if ((itr->second->body.empty() && !itr->second->mailTemplateId) ||
            itr->second->deliver_time > GameTime::GetGameTime() || (itr->second->checked & MAIL_CHECK_MASK_COPIED))
            return MAIL_ERR_INTERNAL_ERROR;
...
bool MailMgr::HandleQueryNextMailTime(Player *player, WorldPacket &data) {
    ...
    for (auto const itr: m_mails) {
        if (itr.second->receiver == player->GetGUID().GetCounter()) {
            ...
            data << uint64(itr.second->messageType == MAIL_NORMAL ? ObjectGuid(HighGuid::Player, itr.second->sender)
...
Item *MailMgr::GetMItem(uint32 id) {
    auto const itr = mMitems.find(id);
...
bool MailMgr::RemoveMItem(uint32 id) {
    return mMitems.erase(id) != 0;
}

// sql helper in func
// set zero money in Mail in db when received money
void MailMgr::_RemoveMoneyFromMail(uint32 mailId, CharacterDatabaseTransaction &trans) {
...
void MailMgr::_LoadMails() {
    ...
    QueryResult result = CharacterDatabase.Query(
            "SELECT id, messageType, stationery, mailTemplateId, sender, receiver, subject, body, has_items, expire_time, deliver_time, money, cod, checked, state FROM mail");
    if (!result) {
    ...
    do {
        ...
        nm->items_exist = fields[8].GetUInt8() == 1;
...
void MailMgr::_LoadMailedItemPointers() {
    ...
    do {
        ...
        if (!proto) {
            TC_LOG_ERROR("mailMgr", "Mail has unknown item_template in mailed items (GUID: {}, Entry: {}), deleted.",
...
void MailMgr::_DeleteExpiryMails(bool startServer) {
    ...
    for (auto const mailItr: m_mails) {
        if (mailItr.second->expire_time <= now) {
            ...
            // if mail with item and COD we should reward sender (he should not lost the money)
            if (mailItr.second->COD && mailItr.second->sender && mailItr.second->messageType == MAIL_NORMAL)
                SendMailByGUID(0, mailItr.second->sender, MAIL_NORMAL, "Money received", "From old mail with COD",
                               mailItr.second->COD, MAIL_CHECK_MASK_COD_PAYMENT);
        }
    }

    if (!mailIds.empty()) {
        CharacterDatabaseTransaction trans = CharacterDatabase.BeginTransaction();
        for (auto itr: mailIds)
            RemoveMail(itr, trans);
...
void MailMgr::_UpdateExpiryTime() {
    ...
    _ExpTimer.Reset(int32(newExpiry - now));
}
