...
void WorldSession::HandleSendMail(WorldPackets::Mail::SendMailClient &sendMail) {
    ...
    if (!receiverGuid) {
        TC_LOG_INFO("network", "Player {} is sending mail to {} (GUID: non-existing!) with subject {} "
                    ...
                    sendMail.Attachments.size(), sendMail.SendMoney, sendMail.Cod, sendMail.StationeryID,
                    sendMail.PackageID);
    ...
    TC_LOG_INFO("network", "Player {} is sending mail to {} ({}) with subject {} and body {} "
                ...
                sendMail.Body, sendMail.Attachments.size(), sendMail.SendMoney, sendMail.Cod, sendMail.StationeryID,
                sendMail.PackageID);
    ...
    int32 cost = !sendMail.Attachments.empty() ? int32(30 * sendMail.Attachments.size()) : 30;  // price hardcoded in client
    ...
    if (!sendMail.Attachments.empty() || sendMail.SendMoney > 0) {
            ...
            for (Item *item: items) {
                if (log) {
                    sLog->OutCommand(GetAccountId(),
                                     "GM {} (GUID: {}) (Account: {}) mail item: {} (Entry: {} Count: {}) "
        ...
        if (log && sendMail.SendMoney > 0) {
            sLog->OutCommand(GetAccountId(), "GM {} (GUID: {}) (Account: {}) mail money: {} to: {} ({}) (Account: {})",
                             GetPlayerName(), GetGUIDLow(), GetAccountId(), sendMail.SendMoney, sendMail.Target,
                             receiverGuid.ToString(), receiverAccountId);
        }
    }
    ...
    if (!sendMail.Attachments.empty())
        sMailMgr->SendMailWithItemsBy(player, receiverGuid.GetCounter(), sendMail.Subject, sendMail.Body,
                                      sendMail.SendMoney, maillist,
                                      sendMail.Body.empty() ? MAIL_CHECK_MASK_COPIED : MAIL_CHECK_MASK_HAS_BODY,
                                      deliver_delay, sendMail.Cod);
    else
        sMailMgr->SendMailBy(player, receiverGuid.GetCounter(), sendMail.Subject, sendMail.Body, sendMail.SendMoney,
                             sendMail.Body.empty() ? MAIL_CHECK_MASK_COPIED : MAIL_CHECK_MASK_HAS_BODY, deliver_delay,
                             sendMail.Cod);
...
void WorldSession::HandleMailTakeItem(WorldPackets::Mail::MailTakeItem &takeItem) {
    ...
    uint32 msg_result = 0;
    uint32 count = 0;
    MailResponseResult result = sMailMgr->HandleMailTakeItem(_player, takeItem.MailID, takeItem.AttachID, count,
                                                             msg_result);
