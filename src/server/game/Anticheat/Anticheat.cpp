...
Anticheat::Anticheat(Player* player)
{
    ...
    m_mountTimer = 0;
    m_rootUpdTimer = 0;
    m_flyhackTimer = sWorld->customGetBoolConfig(CONFIG_AFH_KICK_ENABLED) ? sWorld->customGetIntConfig(CONFIG_ANTICHEAT_FLYHACK_TIMER) : 0;
    m_BaseFlyhackTimer = m_flyhackTimer;
...
void Anticheat::update(uint32 p_time)
{
    if (m_flyhackTimer > 0)
    {
        if (p_time >= m_flyhackTimer)
        {
            ...
            m_flyhackTimer = m_BaseFlyhackTimer;
        }
        else
